        ...
        {models.length > 0 ? (
          <select value={config.model} onChange={(e) => set({ model: e.target.value })}>
            {!models.includes(config.model) && config.model && (
              <option value={config.model}>{config.model} (roʻyxatda yoʻq)</option>
            )}
            {models.map((m) => (
...
export function SettingsPanel(): React.JSX.Element {
  ...
  const ollamaEndpoint = settings?.translate.provider === 'ollama' ? settings.translate.endpoint : null
  useEffect(() => {
    if (!ollamaEndpoint) return
    window.api.listOllamaModels(ollamaEndpoint).then((r) => {
      setOllamaModels(r.models)
      setOllamaError(
        r.ok ? null : `Ollama topilmadi (${ollamaEndpoint}). Ollama oʻrnatilganmi va ishlayaptimi?`
      )
    })
  }, [ollamaEndpoint])
    ...
    window.api
      ...
      .then((all) => {
        const filtered = all
          ...
          .map((v) => v.name)
        setVoices(filtered.length > 0 ? filtered : all.map((v) => v.name))
        setVoicesError(null)
      })
